...
function [dataMat, hdrClip, bndsDdWc, bndsIndWc] = read_PRISM_bin_data(pathWcData, hdrRaw, bits, gridDd, metVar, cropSide)    
...
elseif bits == 16
    ...
    error('read_PRISM_bin_data:unknownBits',['The number of bits is ' num2str(bits) ', which has not been programmed for.']);
end

wcVec = fread(fidWcData, type);  %Read WC binary file.
fclose(fidWcData);
...
if regexpbl(metVar,{'tmn','tmean','tmp','tmx'}) 
    %PRISM temperature data distributed in units of degrees Celsius
%     dataMat = dataMat / 10;
elseif regexpbl(metVar,{'pr'}) || regexpbl(metVar,{'alt','elev','DEM'})
    %PRISM precip data distributed in units of mm
    %Do nothing
else
    warning('read_PRISM_bin_data:metVar',['An unknown meteorological '...
