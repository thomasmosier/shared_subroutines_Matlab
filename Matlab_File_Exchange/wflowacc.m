...
if nargin<3;
    ...
    if ~all(size(Y) == size(X)) && any(size(Y) == 1) && any(size(X) == 1)
        [X, Y] = meshgrid(X, Y);
...
params.coord       = {'geographic', 'projected'};
...
switch params.coord
    case 'geographic'; %Units of degrees lon, lat
        e = double((dem(ic1)-dem(icd1))./haversine_neighbors(X,Y,6371000));
    otherwise
        e = (dem(ic1)-dem(icd1))./hypot(X(ic1)-X(icd1),Y(ic1)-Y(icd1));
end

if nargout > 2;
    S   = sparse(ic1,icd1,e,nrc,nrc); % slope matrix (rise over run)
end
...
% *********************************************************************
% flow direction matrix (refined throughout)
...
switch params.edges
    case 'open';
        flowacc = single((speye(nrc,nrc)-spdiags(edgecorrection,0,nrc,nrc)*M')\params.W0(:));
    otherwise
        flowacc = single((speye(nrc,nrc)-M')\params.W0(:));
