...
disp(' ')
disp('fsar2 Estimation Results using Chebyshev lndet approximation and likelihood dominance inference');
mprint(ests1, info)
disp(' ')
...
tic
 [bmax2, srds2, prhighers2, emax2, logliks2]=fsar2(x, y, d, detvalz,100);
time_with_lndet_addlpoints=toc;

ests2=[bmax srds prhighers];
disp('fsar2 Estimation Results using exact lndet and additional interpolation points');
mprint(ests2, info)
disp(' ')

time_vec=[weight_matrix_time;log_det_time;time_sans_lndet;time_with_lndet;time_with_lndet_addlpoints];
info1.rnames=strvcat('Times','Weight Matrix','Lndet','Approximate sar','Exact sar', 'Exact sar +');
disp('Times (must add lndet time to exact sar or exact sar with added points to compare to approximate sar)');
...
disp('interpolation points did not add more accuracy, showing the default number of interpolation');
disp('points (21) for beta was adequate.');
disp(' ')
disp(' ')
