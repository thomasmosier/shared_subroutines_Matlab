function hh = rotateXLabels( axis, angle, varargin )
...
if ~verLessThan('matlab','8.4.0')
    set(axis, 'XTickLabelRotation', angle)
    if nargout > 1
        hh = [];
    end
    return;
...
[vals, labels] = findAndClearExistingLabels( axis, maxStringLength );

% Create the new label texts
h = createNewLabels( axis, vals, labels, angle );

% Reposition the axes itself to leave space for the new labels
repositionAxes( axis );

% If an X-label is present, move it too
repositionXLabel( axis );

% Store angle
setappdata( axis, 'RotateXLabelsAngle', angle );
...
%-------------------------------------------------------------------------%
    function updateFont()
        ...
        propertyValues = get( axis, properties );
        textLabels = findall( axis, 'Tag', 'RotatedXTickLabel' );
    ...
    function updateAlignment()
        ...
        if strcmpi( get( axis, 'XAxisLocation' ), 'Top' )
            if 0 <= angle && angle < 5
...
%-------------------------------------------------------------------------%
    function onAxesPositionChanged( ~, ~ )
        ...
        % redrawing
        if isappdata( axis, 'OriginalAxesPosition' )
            rmappdata( axis, 'OriginalAxesPosition' );
        end
        if isappdata( axis, 'OriginalXLabelPosition' )
            rmappdata( axis, 'OriginalXLabelPosition' );
        end
        repositionAxes( axis );
        repositionXLabel( axis );
