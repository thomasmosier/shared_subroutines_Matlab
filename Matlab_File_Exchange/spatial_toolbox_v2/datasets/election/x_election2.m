...
%%%%%%%%%%%%% delaunay weight matrix and exact log-determinant %%%%%%%%%%%%
...
%exact lndet computations
tic;
[detvalz]=fdet_interp2(d);
exact_lndet_time=toc;
...
tic
 [bmax4, srds4, prhighers4, emax4, logliks4]=fmix2(x, y, d, detvalz);
mix_time=toc;
...
disp('Comparing OLS, closest neighbor (NN), MESS, and Mixed parameter estimates');
disp(' ');
allests=[bols_aspatial  cbmax  bmax3  bmax4];
allsrds=[ tols_aspatial  csrds  srds3 srds4];
mprint(allests,info)
disp(' ');
disp('Comparing OLS, closest neighbor (NN), MESS, Mixed signed root deviances (SRDs)');
disp(' ');
mprint(allsrds,info1)
disp(' ');
disp('The closest neighbor approach is intermediate to a non-spatial approach (OLS) and a full spatial approach (MESS)');
disp('or the approximate mixed routine. Note, the close agreement between MESS and the mixed routine.');
disp('Note, OLS in this case uses the spatial averages of the basic independent variables as');
disp('additional independent variables.');
disp(' ');


tmat=[ols_time;closest_ar_time;MESS_time;exact_lndet_time;mix_time;doubly_stochastic_time];
disp(' ' );
disp('The following table shows the timings in seconds for the routines.');
disp('Need to add exact log-det time to mix time to compare to MESS or closest NN approach.');
disp(' ');

%making a table using James LeSage's mprint routine at
%www.spatial-econometrics.com
info2.rnames=strvcat('ESTIMATION TIME TABLE: ', '  OLS','  Closest AR', '  MESS','  Exact Log-det', '  Mix','  Doubly Stochastic Scaling');
mprint(tmat,info2);
