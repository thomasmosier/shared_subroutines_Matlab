...
if isequal(fname(end-3:end), '.shp')
    fname = fname(1:end-4);
end
...
if fidl==-1
 error('mShaperead:noFile', ['Cannot file filename: ' fname '.dbf']);
 dbf={};
 fnam={};
 nfield=0;
 mdat=[NaN NaN NaN];
  ...
  for k=1:nrec
    ...
    for l=1:nfield
      ...
      switch ftype(l)
	    ...
	    otherwise
          error('mShaperead:unknownFileType', ['Unknown field type: ' ftype(l)]);
      end
    end
  end 
  fprintf('\n');
...
for k=1:lrec
  ...
  switch stype
    ...
    otherwise
      error('mShaperead:unknownRecordType', ['Unknown record type: ' int2str(stype)]);
  end
end
fclose(fidl);
